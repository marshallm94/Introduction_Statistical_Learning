...
In the (alternate) equation for the Lasso...

$$
\sum_{i=1}^n\left( y_k - \beta_0 - \sum_{j=1}^p\beta_jx_{ij} \right) subject~to~\sum_{j=1}^p \left| \beta_j \right| \leq s
$$
...
In the equation for Ridge Regression...

$$
\sum_{i=1}^n\left( y_k - \beta_0 - \sum_{j=1}^p\beta_jx_{ij} \right) + \lambda\sum_{j=1}^p \beta_j^2
$$
...
* **B**. ...the testing RSS will, *ii*, decrease initially, and then eventually start increasing in a U shape. As $\lambda$ increases from 0, increasingly strict restrictions are put on the magnitude that the regression coefficients can grow too. This has the affect of making the model less flexible and more generalizable, **up to a point**. The left side of the U represents the model's increase in bias being less than it's decrease in variance, and the right side of the U represents the decrease in variance no longer being worth the increase in bias.
