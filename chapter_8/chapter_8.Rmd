...
GE$timestep_1 <- lag(GE$GE.Close, -1)
GE$Direction <- ifelse(GE$timestep_1 > GE$GE.Close, 1, 0)
...
train.idx <- sample(c(TRUE, FALSE), 
                    ...
                    nrow(GE),
                    prob = c(0.75, 0.25))
ge.train <- GE[train.idx, ]
ge.test <- GE[!train.idx, ]

# logistic regression
log.mod <- glm(Direction ~ . - timestep_1,
               data = ge.train,
               family = 'binomial')
...
ggplot(data.frame(x = thresholds,
                  y = errors), aes(x, y)) +
    geom_line(color = 'royalblue1') +
    ggtitle("Accuracy for Various Thresholds") +
    ylab("Accuracy") +
    xlab("Threshold")
```