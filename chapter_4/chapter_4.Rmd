...
ggplot(data = Weekly, aes(x = Year, y = Volume)) +
    ...
    labs(x = 'Year', y = "Volume (Billions)", title = 'Exponential Growth in Trading Volume')
...
ggplot(year_df, aes(x = Year, y = stand_dev)) +
    ...
    labs(x = 'Year', y = 'Standard Deviation', title = 'Trading Volume Variance Increases')
...
```{r}
fit <- glm(Direction ~ Lag1 + Lag2 + Lag3 + Lag4 + Lag5 + Volume, Weekly, family = 'binomial')
summary(fit)
```
...
```{r}
library(leaps)
best_subset_selection <- regsubsets(Direction ~ . - Today, data = train, method = 'exhaustive', nvmax = dim(train)[2] - 2)
...
rownames(results_df) <- c("Direction ~ Lag1",
                          "Direction ~ Lag1 + Lag2",
                          "Direction ~ Lag1 + Lag2 + Volume",
                          "Direction ~ Lag1 + Lag2 + Volume + Lag5",
                          "Direction ~ Lag1 + Lag2 + Volume + Lag5 + Lag4",
                          "Direction ~ Lag1 + Lag2 + Volume + Lag5 + Lag4 + Lag3",
                          "Direction ~ Lag1 + Lag2 + Volume + Lag5 + Lag4 + Lag3 + Year")
...
ggplot(df, aes(y = cylinders,
    ...
    labs(x = 'Cylinders', y = 'Proportion', title = "Most Vehicles Above Median Have 4 Cylinders")
...
ggplot(df, aes(y = mpg01,
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_discrete(name = "Median Status", labels = c('Under Median','Above Median')) +
    labs(x = 'Median Status', y = 'Proportion', title = "Higher Cylinder Vehicles Have Higher Proportion Over Median")
...
ggplot(df, aes(y = origin,
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_discrete(guide = FALSE) +
    labs(x = 'Country of Manufacture', y = 'Proportion', title = "Most Vehicles Under Median Made in US")
...
ggplot(df, aes(y = mpg01,
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_discrete(guide = FALSE) +
    labs(x = 'Country of Manufacture', y = 'Proportion', title = "Most Vehicles Made in Europe or Japan Above Median")
```
...
* **G**. Once again the accuracy and testing error rate are the same as the other algorithms, regardless of $K$. However, with the other algorithms there were 6 False Positives and 3 False Negatives and with KNN has the opposite, 3 False Positives and 6 False Negatives.
...
nn_train <- function(k) {
    nearest_neighbors <- knn(data.frame(knn_train),
                             ...
                             train$mpg01,
                             k=k)
    cf <- confusionMatrix(factor(test$mpg01), nearest_neighbors, '1')
    print(cf$table)
    print(cf$overall[1])
}
