...
```{r}
library(MASS)
lda <- lda(Direction ~ Lag2, train)
test$prediction <- predict(lda, newdata = test, type = 'response')$class
...
```{r}
library(MASS)
qda <- qda(Direction ~ Lag2, train)
test$prediction <- predict(qda, newdata = test, type = 'response')$class
cf <- confusionMatrix(factor(test$prediction), factor(test$Direction), 'Up')
cf$table
cf$overall[1]
```
...
for (model in models) {
    # logistic regression
    fit <- glm(model, train, family = 'binomial')
    lr_acc <- get_accuracy(fit)[[2]]
    
    # LDA
    lda <- lda(model, train)
    lda_y_hat <- predict(lda, newdata = test, type = 'response')$class
    cf <- confusionMatrix(factor(lda_y_hat), factor(test$Direction), 'Up')
    lda_acc <- cf$overall[1]
    
    # QDA
    qda <- qda(model, train)
    qda_y_hat <- predict(qda, newdata = test, type = 'response')$class
