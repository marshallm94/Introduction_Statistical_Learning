...
Algebraically, this is proved by:

\begin{align}
\frac{1}{2}\sum_{i,i' \in C_k}\sum_{j=1}^P \left( x_{i,j} - x_{i',j} \right)^2 & = 2\sum_{i \in C_k}\sum_{j=1}^P \left( x_{i,j} - \bar{x_{k,j}} \right)^2 \\
\frac{1}{2}\sum_{i,i' \in C_k}\sum_{j=1}^P \left( x_{i,j} - \bar{x_{k,j}} - x_{i',j} + \bar{x_{k,j}} \right)^2 & = \\
\frac{1}{2}\sum_{i,i' \in C_k}\sum_{j=1}^P \left( (x_{i,j} - \bar{x_{k,j}}) - (x_{i',j} - \bar{x_{k,j}}) \right)^2 & = \\
\frac{1}{2}\sum_{i,i' \in C_k}\sum_{j=1}^P \left( (x_{i,j} - \bar{x_{k,j}})^2 - 2(x_{i',j} - \bar{x_{k,j}})(x_{i,j} - \bar{x_{k,j}}) + (x_{i',j} - \bar{x_{k,j}})^2 \right) & = \\
\\
\\
...
\frac{1}{\lvert C_k \rvert}\sum_{i,i' \in C_k}\sum_{j=1}^P \left( x_{i,j}^2 - 2x_{i',j}x_{i,j} + x_{i',j}^2 \right) & = 2\sum_{i \in C_k}\sum_{j=1}^P \left( x_{i,j}^2 - 2\bar{x_{k,j}}x_{i,j} + \bar{x_{k,j}}^2 \right) \\
\frac{1}{\lvert C_k \rvert}\sum_{i,i' \in C_k}\sum_{j=1}^P x_{i,j}^2 - \frac{1}{\lvert C_k \rvert}\sum_{i,i' \in C_k}\sum_{j=1}^P2x_{i',j}x_{i,j} + \frac{1}{\lvert C_k \rvert}\sum_{i,i' \in C_k}\sum_{j=1}^Px_{i',j}^2 & = \\
\sum_{i \in C_k}\sum_{j=1}^P \frac{1}{\lvert C_k \rvert}x_{i,j}^2 - \sum_{i,i' \in C_k}\sum_{j=1}^P \frac{1}{\lvert C_k \rvert} 2x_{i',j}x_{i,j} + \sum_{i' \in C_k}\sum_{j=1}^P \frac{1}{\lvert C_k \rvert} x_{i',j}^2 & = \\
\sum_{i \in C_k}\sum_{j=1}^P \frac{1}{\lvert C_k \rvert}x_{i,j}^2 - \sum_{i,i' \in C_k}\sum_{j=1}^P 2x_{i',j}\bar{x_{k,j}} + \sum_{i' \in C_k}\sum_{j=1}^P \frac{1}{\lvert C_k \rvert} x_{i',j}^2 & = \\
\frac{1}{\lvert C_k \rvert}\sum_{i \in C_k}\sum_{j=1}^P x_{i,j}^2 - 2\sum_{i,i' \in C_k}\sum_{j=1}^P x_{i',j}\bar{x_{k,j}} + \frac{1}{\lvert C_k \rvert}\sum_{i' \in C_k}\sum_{j=1}^P x_{i',j}^2 & = \\
\frac{2}{\lvert C_k \rvert}\sum_{i \in C_k}\sum_{j=1}^P x_{i,j}^2 - 2\sum_{i' \in C_k}\sum_{j=1}^P x_{i',j}\bar{x_{k,j}} & = \\
\frac{2}{\lvert C_k \rvert}\sum_{i \in C_k}\sum_{j=1}^P x_{i,j}^2 - 2\sum_{i' \in C_k}\sum_{j=1}^P x_{i',j}\bar{x_{k,j}} & = \\
\end{align}
